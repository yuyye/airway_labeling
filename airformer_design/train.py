...
import os
import numpy as np
from transformer import AirwayFormer_multitask_se
...
save_dir = "checkpoints/2task_mlpsame_alpha_{}_{}_spd_seed{}/".format(alpha1,alpha2,seed)
...
my_net = AirwayFormer_multitask_se(input_dim=23, num_classes1=6, num_classes2=20, num_classes3=127, dim=128, heads=4,
...
for epoch in range(epochs):
    ...
    for case in train_loader_case:
        ...
        output1, output2, output3,output12,output23= my_net(x, spd.detach(), 0.1)
        ...
        loss = loss_function(output1, y_lobar) + loss_function(output2, y_seg) + loss_function(output3, y_subseg)+\
               alpha1 * loss_function(output12, y_seg) + alpha2 * loss_function(output23,y_subseg)
    ...
    if (epoch + 1) % 10 == 0:
        ...
        for case in test_loader_case:
            ...
            pred1, pred2, pred3,_,_ = my_net(x, spd.detach(), 0.1)
