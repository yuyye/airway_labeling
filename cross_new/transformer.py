    ...
    def forward(self, x,spd,x2,A,p):
        ...
        for i in range(len(self.transformer)):
            ...
            if i == 2:
                ...
                x = self.transformer[i](x, spd[i],x2[i], A,p)
            x_.append(x)
            list.append(x)
...
class AirwayFormer_att_se(nn.Module):
    def __init__(self, input_dim, num_classes1, num_classes2, num_classes3, dim, heads, mlp_dim, dim_head=64,
        ...
        self.spatial_pos_encoder1_gen = nn.Embedding(30, 1, padding_idx=0)
        self.spatial_pos_encoder2_gen = nn.Embedding(30, 1, padding_idx=0)
        self.spatial_pos_encoder3_gen = nn.Embedding(30, 1, padding_idx=0)

        self.spatial_pos_encoder1_lca = nn.Embedding(30, 1, padding_idx=0)
        self.spatial_pos_encoder2_lca = nn.Embedding(30, 1, padding_idx=0)
        self.spatial_pos_encoder3_lca = nn.Embedding(30, 1, padding_idx=0)




    def forward(self,x,spd,gen,lca,A,p):
        x_raw = x
        ...
        lca = lca.unsqueeze(0)
        dict = []
        dict1 = torch.cat((self.spatial_pos_encoder1_spd(spd),self.spatial_pos_encoder1_gen(gen),self.spatial_pos_encoder1_lca(lca)),-1).permute(0, 3, 1, 2)
        dict.append(dict1)
        dict2 = torch.cat((self.spatial_pos_encoder2_spd(spd),self.spatial_pos_encoder2_gen(gen),self.spatial_pos_encoder2_lca(lca)),-1).permute(0, 3, 1, 2)
        dict.append(dict2)
        dict3 = torch.cat((self.spatial_pos_encoder3_spd(spd),self.spatial_pos_encoder3_gen(gen),self.spatial_pos_encoder3_lca(lca)),-1).permute(0, 3, 1, 2)
        dict.append(dict3)
        ...
        outx_31=x3_1
        x2 = [x2_1,x3_1]
        x1_2, x2_2, x3_2 = self.accecpt(x,dict,x2,A,p)
        out_x22 = x2_2
        out_x32 = x3_2
        ...
        x3_2= x3_2.squeeze(0) #x_raw,out_x21,outx_31,out_x22,out_x32,
