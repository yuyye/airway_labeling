...
class Attention_att(nn.Module):
    ...
    def forward(self, x, att,p,alpha):
        ...
        dots = torch.matmul(q, k.transpose(-1, -2)) * self.scale
        #矩阵归一化         torch.norm(dots, p=2, dim=-1,keepdim=True)
        #dots = (1 - alpha) * F.normalize(att, p=2, dim=-1) * torch.norm(dots, p=2, dim=-1,keepdim=True)+ alpha*dots
        dots = (1 - alpha) *att +alpha *  F.normalize(dots, p=2, dim=-1)*torch.norm(att, p=2, dim=-1,keepdim=True)
        #dots = (1-alpha) * F.normalize(att, p=2, dim=-1) +alpha *  F.normalize(torch.matmul(q, k.transpose(-1, -2)) * self.scale, p=2, dim=-1)
...
class AirwayFormer_att_normal(nn.Module):
    def __init__(self, input_dim, num_classes1, num_classes2, num_classes3, dim, heads, mlp_dim, dim_head=64,
